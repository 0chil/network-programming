...
const unsigned char *print_name2(const unsigned char *msg, const unsigned char *p, const unsigned char *end)
{
    if ((*p & 0xC0) == 0xC0)
    {
        int ptr = (*p & 0x3f) << 8 + p[1];
        p += 2;
        print_name2(msg, msg + ptr, end);
        return p;
    }
    else
    {
